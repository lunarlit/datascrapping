...
```python
weekly = "    2주차 - 파이썬 기초부터 첫 데이터 수집까지    "

print(weekly, '/')
```

```text
출력 결과 :    2주차 - 파이썬 기초부터 첫 데이터 수집까지     /
```

수집 결과 들어온 텍스트가 좌우에 여백이 남아있고, "2주차 - 파이썬..." 를 "2. 파이썬..." 와 같이 포매팅하고 싶은 상황입니다. 텍스트가 어디서 끝나는지 알아보기 위해 마지막에 '/' 를 출력해주었는데, 좌우에 여백이 그대로 출력되는 것을 볼 수 있습니다.
...
```python
weekly = weekly.replace('주차 - ', '. ')

print(weekly, '/')
```

```text
출력 결과 :    2. 파이썬 기초부터 첫 데이터 수집까지     /
```

weekly. 으로 시작하는 문법은 처음보는 것이지만 잠시 설명을 미루겠습니다.

replace\(\) 함수는 두 개의 텍스트\(old, new\)를 입력받아  "." 이전에 있는 텍스트에서 old와 일치하는 부분을 new로 바꾸어 줍니다. 

그리고 바뀐 텍스트를 결과값으로 돌려줍니다.

여기서는 '주차 - ' 라는 부분을 찾아 '. '으로 바꾸어주었네요. 유용하죠?
...
{% hint style="warning" %}
두 함수 모두 텍스트 자체를 직접 수정하는 게 아니라, 수정된 텍스트를 결과로 돌려줍니다.

따라서 weekly = weekly.함수\(\) 를 통해 weekly의 값을 결과값으로 바꾸어주고 있습니다.

이렇게 할당하지 않고 단순히 함수만 호출한다면 weekly의 값은 수정되지 않습니다.
{% endhint %}
...
여러분이 .replace\(\)나 .strip\(\) 함수를 사용할 때 그 내부가 어떻게 구현되었는지 전혀 신경쓸 필요가 없는 것처럼 말이죠.

따라서 코드를 읽을 다른 사람이나, 기억을 상실할 미래의 나를 위해 함수를 자주 작성하고 이름을 잘 지어주는 것이 좋습니다.
...
를 선언하는 순간, weekly 변수는 String\(문자열\) 이 되어 String을 조작할 수 있는 수많은 함수가 모여있는 공간에 접근할 수 있게 됩니다.
